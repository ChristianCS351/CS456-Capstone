...
if (!isset($_SESSION['username'])) {
    die("You must log in to view your shopping list.");
}

$cleanUsername = preg_replace("/[^a-zA-Z0-9]/", "_", strtolower($_SESSION['username']));
$shopTable = $cleanUsername . "_shop";

// INSERT ITEM INTO USER'S SHOP TABLE
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['list'])) {
    $name     = trim($_POST['item_name']);
    $quantity = (int)$_POST['qty'];

    if ($name !== "" && $quantity > 0) {
        $stmt = $conn->prepare("INSERT INTO `$shopTable` (name, quantity) VALUES (?, ?)");
        $stmt->bind_param("si", $name, $quantity);

        if ($stmt->execute()) {
            $messageList = "Item added to your shopping list.";
        } else {
            $messageList = "Error adding item: " . $conn->error;
        }
...
$listItems = [];
$result = $conn->query("SELECT id, name, quantity FROM `$shopTable` ORDER BY id ASC");
if ($result) {
...
<head>
    ...
    <style>
        ...
        /* hide buttons/nav/header during print */
...
<div class="main-content">
    ...
    <!-- 2) Wrap JUST the shopping list section with an id="print_area" -->
...
$(document).ready(function(){
    $('.hero-slide').slick({
        ...
        <!-- Add these two script tags at the VERY bottom before </body> -->
